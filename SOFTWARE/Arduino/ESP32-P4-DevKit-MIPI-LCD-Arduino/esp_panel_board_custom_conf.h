...
// Display revision selector. Leave this at 2 for WLK2802MIPI-15P-V2.
// Set it to 1 for the original WLK2802MIPI-15P, which has a PCA9536 at 0x41.
 ...
 */
#define ESP_PANEL_BOARD_NAME                "Olimex:ESP32-P4-DevKit"

/**
 * @brief Panel resolution configuration in pixels
 */
#define ESP_PANEL_BOARD_WIDTH               (480)   // Panel width (horizontal, in pixels)
#define ESP_PANEL_BOARD_HEIGHT              (640)   // Panel height (vertical, in pixels)
 ...
 */
    ...
    #ifndef OLIMEX_MIPI_DSI_LANE_RATE_MBPS
        // Olimex's ESP-IDF demo uses 1000 Mbps. Start lower while debugging
        // corrupted hardware DSI patterns, then raise it once the link is clean.
        #define OLIMEX_MIPI_DSI_LANE_RATE_MBPS             (500)
    #endif
    #define ESP_PANEL_BOARD_LCD_MIPI_DSI_LANE_RATE_MBPS     OLIMEX_MIPI_DSI_LANE_RATE_MBPS
    ...
    #ifndef OLIMEX_MIPI_DPI_CLK_MHZ
        // Olimex's ESP-IDF demo uses 25 MHz. 16 MHz keeps the same timings but
        // lowers the frame rate and DSI payload while checking signal margin.
        #define OLIMEX_MIPI_DPI_CLK_MHZ                    (16)
    #endif
    #define ESP_PANEL_BOARD_LCD_MIPI_DPI_CLK_MHZ            OLIMEX_MIPI_DPI_CLK_MHZ
    ...
    #define ESP_PANEL_BOARD_LCD_MIPI_DPI_VBP                (14)
    #define ESP_PANEL_BOARD_LCD_MIPI_DPI_VFP                (8)
    /* For DSI power PHY */
 ...
 */
#define ESP_PANEL_BOARD_USE_BACKLIGHT           (OLIMEX_MIPI_LCD_VERSION == 1)
 ...
 */
#define ESP_PANEL_BOARD_USE_EXPANDER            (OLIMEX_MIPI_LCD_VERSION == 1)
...
/* For device */
#define ESP_PANEL_BOARD_EXPANDER_I2C_ADDRESS        (0x41)  // The actual I2C address. Even for the same model of IC,
...
#define ESP_PANEL_BOARD_LCD_PRE_BEGIN_FUNCTION(p) \
    {  \
        ...
        if (!expander->pinMode(3, OUTPUT)) return false; \
        if (!expander->digitalWrite(3, LOW)) return false; \
        vTaskDelay(pdMS_TO_TICKS(10)); \
        if (!expander->digitalWrite(3, HIGH)) return false; \
        vTaskDelay(pdMS_TO_TICKS(10)); \
        if (!expander->digitalWrite(3, LOW)) return false; \
        if (!expander->pinMode(3, INPUT)) return false; \
        vTaskDelay(pdMS_TO_TICKS(120)); \
        printf("V1 PCA9536 reset complete (0x41, pin 3)\n"); \
        return true;    \
    }
#endif
 ...
 */
 ...
 #define ESP_PANEL_BOARD_LCD_POST_BEGIN_FUNCTION(p) \
    {  \
        auto board = static_cast<Board *>(p);  \
        auto panel = board->getLCD()->getRefreshPanelHandle(); \
        esp_lcd_panel_disp_sleep(panel, false); \
        esp_lcd_panel_disp_on_off(panel, true); \
        printf("ESP_PANEL_BOARD_LCD_POST_BEGIN_FUNCTION\n"); \
        return true;    \
    }
